...
SELECT Physician.IDNumber,PhysicianName, COUNT(Physician.IDNumber)
FROM Physician ,  TimeCard
WHERE Physician.IDNumber != Timecard.PhysicianID--check
GROUP BY Physician.IDNumber, PhysicianName
...
FROM Nurse, Bed
WHERE Salary > 70000
AND Nurse.IDNumber = Bed.NurseNumber
GROUP BY Nurse.IDNumber;
...
SELECT Specialty, COUNT(Physician.IDNumber), SUM(Timecard.TimeHours)
FROM Physician, Timecard
